...
from . import report
from .backtest import Variant, run_variant, bench_monthly, current_picks, rebalance_schedule
...
def rank_at(close, sig_df, vol_df, date, n, weighting):
    """Top-n tickers by signal at a given date (used for held/new status)."""
    s = sig_df.loc[date].dropna()
    s = s[close.loc[date].reindex(s.index).notna()]
    if weighting == "inv_vol":
        s = s[vol_df.loc[date].reindex(s.index).notna()]
...
def main() -> None:
    ...
    for key, tcfg in cfg["tracks"].items():
        n, weighting = int(tcfg["n"]), tcfg["weighting"]
        log.info("track %s: %d variants (n=%d, %s)", key, len(variants), n, weighting)
        results = {}
        for v in variants:
            res = run_variant(pdata, v, n, weighting, start_bt, cost_bps, atr_mult, pre)
            results[v.name] = res
        ...
        prev_sd = pairs[-1][0]
        prev_variant = sel.selections.iloc[-1]
        prev_set = rank_at(close, pre[("sig", prev_variant.partition("+")[0])], pre["vol"],
                           prev_sd, n, weighting)

        live_sig = sel.live_variant.partition("+")[0]
        picks = current_picks(pdata, live_sig, n, weighting, atr_mult, prev_set)
